...
let providerCalls = 0;
const env = new Map([['SUPABASE_URL','https://fixture.supabase.co'],['SUPABASE_ANON_KEY','fixture-key'],['GROQ_API_KEY','groq-test-key'],['OPENAI_API_KEY','openai-test-key']]);
...
globalThis.fetch=async(url,init)=>{
 ...
 const isStay=input.kind==='stays';
 assert.equal(path,isStay?'https://api.openai.com/v1/chat/completions':'https://api.groq.com/openai/v1/chat/completions');
 assert.equal(init.headers.Authorization,isStay?'Bearer openai-test-key':'Bearer groq-test-key');
 if(isStay){assert.equal(payload.model,env.get('OPENAI_STAYS_MODEL')||'gpt-4.1-mini');assert.equal(payload.store,false);assert.equal(payload.reasoning_effort,undefined);assert.equal(payload.response_format.json_schema.strict,true);}
 providerCalls++;
...
providerStatus=401;const rejected=await request('stays');assert.equal(rejected.status,503);assert.ok(!rejected.body.error.includes('private'));console.log('PASS OpenAI rejection stays private');
providerStatus=429;const stayLimited=await request('stays');assert.equal(stayLimited.status,429);console.log('PASS OpenAI rate limit');
providerStatus=200;env.set('OPENAI_STAYS_MODEL','gpt-4.1-mini-2025-04-14');assert.equal((await request('stays')).status,200);env.delete('OPENAI_STAYS_MODEL');
env.delete('GROQ_API_KEY');assert.equal((await request('stays')).status,200);env.set('GROQ_API_KEY','groq-test-key');console.log('PASS stays work without Groq credentials');
providerStatus=200;env.delete('OPENAI_API_KEY');const unconfigured=await request('stays');assert.equal(unconfigured.status,503);assert.match(unconfigured.body.error,/not configured/);console.log('PASS configuration error reaches app safely');
