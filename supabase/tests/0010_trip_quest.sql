...
select is(pg_temp.room()->>'stage', 'budget', 'a valid shared period unlocks private budgets');
select throws_ok($$select pg_temp.act('{"type":"budget","comfortableBudgetMYR":0,"maxBudgetMYR":0}')$$, '22023', 'Budgets must be whole MYR from 1 to 1,000,000, with maximum at least comfortable spending.', 'zero budget is rejected');
select throws_ok($$select pg_temp.act('{"type":"budget","comfortableBudgetMYR":1500.5,"maxBudgetMYR":1500.5}')$$, '22023', 'Budgets must be whole MYR from 1 to 1,000,000, with maximum at least comfortable spending.', 'fractional budget is rejected');
select throws_ok($$select pg_temp.act('{"type":"budget","comfortableBudgetMYR":1000001,"maxBudgetMYR":1000001}')$$, '22023', 'Budgets must be whole MYR from 1 to 1,000,000, with maximum at least comfortable spending.', 'budget maximum is enforced');
select pg_temp.act('{"type":"budget","comfortableBudgetMYR":4000,"maxBudgetMYR":4000}');
...
select set_config('request.jwt.claim.sub', '22222222-2222-4222-8222-222222222222', true);
select pg_temp.act('{"type":"budget","comfortableBudgetMYR":1500,"maxBudgetMYR":1500}');
select set_config('request.jwt.claim.sub', '33333333-3333-4333-8333-333333333333', true);
select pg_temp.act('{"type":"budget","comfortableBudgetMYR":4500,"maxBudgetMYR":5000}');
select set_config('request.jwt.claim.sub', '22222222-2222-4222-8222-222222222222', true);
select is((pg_temp.room()->'budgetSummary'->>'crewHardCeiling')::integer, 1500, 'group ceiling is the lowest member maximum, never the average');
...
select is(pg_temp.room()->>'stage', 'picks', 'budget advances to Wishlist');
...
select throws_ok($$select pg_temp.act('{"type":"budget","comfortableBudgetMYR":100,"maxBudgetMYR":100}', 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb')$$, '22023', 'This request identifier was already used for a different quest action.', 'idempotency keys cannot be reused with different payloads');
select throws_ok($$select pg_temp.act('{"type":"budget","comfortableBudgetMYR":100,"maxBudgetMYR":100}')$$, '22023', 'Budgets can be submitted during the budget stage.', 'completed plans are immutable');
...
select throws_ok($$select pg_temp.act('{"type":"attraction_votes","attractionIds":["th-wat-arun"]}')$$, '22023', 'Choose at least one attraction in the selected country.', 'map selection rejects attractions in another country');
select throws_ok($$select pg_temp.act('{"type":"attraction_votes","attractionIds":["my-invented-place"]}')$$, '22023', 'Choose at least one attraction in the selected country.', 'map selection rejects fictional attraction identifiers');
select pg_temp.act('{"type":"attraction_votes","attractionIds":["my-george-town"]}');
select set_config('request.jwt.claim.sub', '22222222-2222-4222-8222-222222222222', true);
select pg_temp.act('{"type":"attraction_votes","attractionIds":["my-george-town"]}');
select set_config('request.jwt.claim.sub', '11111111-1111-4111-8111-111111111111', true);
select pg_temp.act('{"type":"compile_attractions"}');
select is(pg_temp.room()->>'stage', 'logistics', 'compiling map attractions opens Logistics');
select set_config('request.jwt.claim.sub', '33333333-3333-4333-8333-333333333333', true);
