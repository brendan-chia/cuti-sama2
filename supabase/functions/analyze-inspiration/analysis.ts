...
export async function analyzeContent(text:string, fetcher=fetch) {
  const provider=Deno.env.get('INSPIRATION_LLM_PROVIDER')||'groq';
  ...
  const model=provider==='openai'?Deno.env.get('OPENAI_INSPIRATION_MODEL'):groq.model;
  ...
  analysis.places=analysis.places.filter(place=>text.toLowerCase().includes(place.evidence.toLowerCase()));
