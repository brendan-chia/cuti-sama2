...
Deno.test('invalid Groq JSON/schema is recoverable', async () => {
  ...
  const malformed = await requestAiWording(facts, { fetchImpl: (() => Promise.resolve(new Response('{"choices":[{"message":{"content":"not-json"}}]}'))) as typeof fetch });
  assert(malformed === null, 'malformed output should fall back');
  const unknown = await requestAiWording(facts, { fetchImpl: (() => Promise.resolve(new Response(JSON.stringify({ choices: [{ message: { content: JSON.stringify({ heading: 'Book Bali', summary: 'Invented', facts: [{ factId: 'invented:bali', wording: 'Book it' }] }) } }] })))) as typeof fetch });
...
Deno.test('Groq timeout is recoverable', async () => {
  ...
  const value = await requestAiWording(facts, { fetchImpl: hangingFetch, timeoutMs: 1 });
