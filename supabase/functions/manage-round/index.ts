...
Deno.serve(async (request) => {
  ...
  const parsed = ManageRoundPayloadSchema.safeParse(await requestJson(request));
  if (!parsed.success) return json({ error: 'Round action is invalid.' }, 400);
  const input = parsed.data;
  const { data, error } = await client.rpc('manage_preference_round', { p_trip_id: input.tripId, p_action: input.action, p_idempotency_key: input.idempotencyKey });
  if (error) {
    if (error.code === '42501') return json({ error: error.message }, 403);
    if (error.code === '22023') return json({ error: error.message }, 409);
    return json({ error: 'Could not manage preference round.' }, 500);
  }
  return json(data);
});
