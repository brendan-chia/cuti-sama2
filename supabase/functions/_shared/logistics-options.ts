...
export async function requestLogisticsOptions(room: LogisticsContext, input: LogisticsRequest) {
  if (input.kind === 'stays' && room.period.startsOn === room.period.endsOn) return { transport: [], stays: [] };
    ...
    if (!apiKey) throw new Error('Travel suggestions are not configured yet.');
    ...
    if (!response.ok) throw new Error(response.status === 429 ? 'AI is receiving too many requests. Please try again in a minute.' : 'Travel suggestions are temporarily unavailable. Try again.');
    const body = await response.json();
    const parsed = answer.safeParse(JSON.parse(body.choices?.[0]?.message?.content ?? '{}'));
    if (!parsed.success) throw new Error('The suggestions were incomplete. Please retry.');
    if (input.kind === 'stays' && new Set(parsed.data.stays.map(stay => stay.category)).size !== 3) throw new Error('Could not find all three accommodation categories. Please retry.');
    ...
    if (!(kind === 'transport' ? transport.length : stays.length)) throw new Error('No suitable options found. Try again or add your own booking.');
