...
Deno.test('only exact candidate IDs become trip periods', async () => {
  ...
  let sentData = '';
  const valid = await requestGroqTripPeriods(candidates, {
    ...options,
...
Deno.test('invalid, invented, or duplicated AI output falls back', async () => {
  for (const output of [
    ...
    const result = await requestGroqTripPeriods(candidates, { ...options, fetchImpl: (() => Promise.resolve(aiResponse(output))) as typeof fetch });
    assert(result === null, 'invalid provider output must not be shown');
  }
  const malformed = await requestGroqTripPeriods(candidates, { ...options, fetchImpl: (() => Promise.resolve(new Response('not-json'))) as typeof fetch });
...
Deno.test('provider errors, missing config, and timeout allow calendar fallback', async () => {
  const unavailable = await requestGroqTripPeriods(candidates, { ...options, fetchImpl: (() => Promise.resolve(new Response('', { status: 503 }))) as typeof fetch });
  assert(unavailable === null, 'provider HTTP failure must fall back');
  const unconfigured = await requestGroqTripPeriods(candidates, { apiKey: '', model: '' });
  ...
  const timeout = await requestGroqTripPeriods(candidates, { ...options, timeoutMs: 1, fetchImpl: hangingFetch });
