...
Deno.serve(async (request) => {
  ...
  if (!operation.data) {
    const inserted = await service.from('itinerary_generation_operations').insert({ trip_id: parsed.data.tripId, requested_by: membership.id, idempotency_key: parsed.data.idempotencyKey }).select('id,status').single();
    if (inserted.error?.code === '23505') operation = await service.from('itinerary_generation_operations').select('id,status').eq('trip_id', parsed.data.tripId).eq('idempotency_key', parsed.data.idempotencyKey).single();
    else operation = inserted;
  } else if (operation.data.status === 'failed') {
    await service.from('itinerary_generation_operations').update({ status: 'pending', error: null, completed_at: null }).eq('id', operation.data.id);
  }
  ...
  if (tripQuery.error || constraintQuery.error || roundQuery.error) { await fail(service, operationId, 'Could not load group inputs.'); return json({ error: 'Could not load group inputs.' }, 500); }
