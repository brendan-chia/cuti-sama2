...
-- Remote 0037 introduced a workspace importer which reads the legacy quest cap.
-- Preserve its behavior while following the renamed hard-maximum column.
do $migration$
declare definition text;
begin
  if to_regprocedure('public.get_trip_workspace(uuid)') is not null then
    definition := pg_get_functiondef('public.get_trip_workspace(uuid)'::regprocedure);
    if position('min(i.budget)' in definition) = 0 then
      raise exception 'Workspace budget importer changed; review its column reference before deploying.';
    end if;
    execute replace(definition, 'min(i.budget)', 'min(i.max_budget_myr)');
  end if;
end;
$migration$;
