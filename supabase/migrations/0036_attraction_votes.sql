-- Each active traveller votes; only the organiser compiles the union.
alter table public.trip_quest_inputs add column attraction_votes text[] not null default '{}';
...
begin
  ...
  return jsonb_build_object(
    ...
    'attractionVotes', (select coalesce(jsonb_agg(jsonb_build_object('memberId', i.member_id, 'attractionIds', i.attraction_votes) order by i.member_id), '[]'::jsonb)
      ...
      where i.trip_id = p_trip_id and m.active and cardinality(i.attraction_votes) > 0), 'ownBudget', own_input.budget,
  ...
  if action_type = 'availability' then
    ...
    if (select travel_party from public.trips where id = p_trip_id) = 'solo' then
      update public.trip_quests set period = jsonb_build_object('startsOn', start_date, 'endsOn', end_date,
        'label', 'Your travel dates', 'reason', 'Chosen for your solo trip.'), stage = 'picks' where trip_id = p_trip_id;
