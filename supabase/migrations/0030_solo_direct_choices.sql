-- Solo travellers choose a destination directly; group ballots are unchanged.
...
begin
  ...
  if (select travel_party from public.trips where id = p_trip_id) = 'solo' then
    if quest.stage = 'picks' and exists (select 1 from public.trip_quest_inputs where trip_id = p_trip_id and cardinality(country_codes) = 1) then
      update public.trip_quests set stage = 'explore', selected_country_code = (select country_codes[1] from public.trip_quest_inputs where trip_id = p_trip_id limit 1), results = '[]', tied_country_codes = '{}', updated_at = now() where trip_id = p_trip_id;
    end if;
    return;
  end if;
...
-- Existing solo ballots return to destination choice; do not choose on their behalf.
update public.trip_quest_inputs i set country_codes = '{}', votes = '{}'
from public.trip_quests q, public.trips t where i.trip_id = q.trip_id and t.id = q.trip_id and t.travel_party = 'solo' and q.stage = 'voting';
update public.trip_quests q set stage = 'picks', countries = '{}', results = '[]', tied_country_codes = '{}', selected_country_code = null, revision = revision + 1
from public.trips t where t.id = q.trip_id and t.travel_party = 'solo' and q.stage = 'voting';
