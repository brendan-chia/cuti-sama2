...
export function ReviseItineraryScreen({ tripId, onBack, onActivated, loadAction = loadRevisionState, reviseAction = reviseItinerary, activateAction = activateItinerary }: Props) {
  ...
  return <Screen testID="revise-itinerary-screen">
    ...
    {!preview ? <>
      <View style={styles.choices}>{choices.map((choice) => <Pressable accessibilityRole="radio" aria-checked={choice.kind === kind} accessibilityState={{ selected: choice.kind === kind }} key={choice.kind} onPress={() => setKind(choice.kind)} style={[styles.choice, choice.kind === kind ? styles.choiceSelected : null]} testID={`revision-${choice.kind}`}><Text style={styles.choiceTitle}>{choice.label}</Text><Text style={styles.body}>{choice.detail}</Text></Pressable>)}</View>
      {kind === 'pace' ? <View style={styles.paces}>{(['relaxed', 'balanced', 'full'] as const).map((value) => <Pressable accessibilityRole="radio" aria-checked={value === pace} accessibilityState={{ selected: value === pace }} key={value} onPress={() => setPace(value)} style={[styles.pace, value === pace ? styles.paceSelected : null]}><Text style={styles.paceText}>{value}</Text></Pressable>)}</View> : null}
