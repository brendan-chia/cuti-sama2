...
export function BudgetStage({ room, busy, act }: { room: QuestRoom; busy: boolean; act: (action: QuestAction) => Promise<boolean> }) {
  ...
  return <View style={s.stack}>
    ...
    <View style={s.panel}>
      ...
      <FormField label={solo ? "My trip budget (MYR)" : "My maximum per person (MYR)"} hint={solo ? "Your limit for the whole trip. Use a whole ringgit amount." : "Your answer is not shown beside your name. Everyone sees the group spending ceiling. Use a whole ringgit amount."} keyboardType="number-pad" maxLength={7} value={value} onChangeText={setValue} placeholder="e.g. 2500" error={value && !valid ? 'Enter a whole amount from RM 1 to RM 1,000,000.' : undefined} />
      {room.ownBudget ? <Text style={s.small}>Your saved limit: {money(room.ownBudget)}</Text> : null}
      <AppButton label={room.ownBudget ? 'Update my budget' : solo ? 'Save my budget' : 'Play my budget card'} testID="submit-quest-budget" disabled={!valid} loading={busy} onPress={() => void act({ type: 'budget', amount })} />
    </View>
    ...
    </View> : <Text style={s.body}>{solo ? 'Save your budget to continue to transport and accommodation.' : 'The group’s spending ceiling unlocks when everyone has played a budget card. We use the lowest limit so no one is pushed above their budget.'}</Text>}
