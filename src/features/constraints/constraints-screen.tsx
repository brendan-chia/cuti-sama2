...
export function ConstraintsScreen({ tripId, onBack, loadAction = loadConstraintCollection, saveAction = saveConstraints, lockAction = lockConstraints, subscribeAction = subscribeToConstraints }: Props) {
  ...
  return <Screen footer={<View style={styles.footer}>
    ...
    <View pointerEvents={locked ? 'none' : 'auto'} style={[styles.form, locked ? styles.readOnly : null]}>
      ...
      <View style={styles.group}><FormField error={errors.startsOn} label={`Earliest start${undecided ? ' *' : ''}`} onChangeText={(text) => update('startsOn', text)} placeholder="YYYY-MM-DD" value={values.startsOn} /><FormField error={errors.endsOn} label={`Latest end${undecided ? ' *' : ''}`} onChangeText={(text) => update('endsOn', text)} placeholder="YYYY-MM-DD" value={values.endsOn} /></View>
      ...
      <View style={styles.group}><FormField error={errors.budgetMin} keyboardType="decimal-pad" label={`Minimum budget${undecided ? ' *' : ''}`} onChangeText={(text) => update('budgetMin', text)} placeholder="800" value={values.budgetMin} /><FormField error={errors.budgetMax} keyboardType="decimal-pad" label={`Maximum budget${undecided ? ' *' : ''}`} onChangeText={(text) => update('budgetMax', text)} placeholder="1500" value={values.budgetMax} /></View>
      <FormField autoCapitalize="characters" error={errors.currency} label={`Currency${undecided ? ' *' : ''}`} maxLength={3} onChangeText={(text) => update('currency', text.toUpperCase())} placeholder="MYR" value={values.currency} />
