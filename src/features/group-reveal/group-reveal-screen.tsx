      ...
      return <View key={section.kind} style={styles.section} testID={`section-${section.kind}`}>
        ...
        {facts.length === 0 ? <Text style={styles.empty}>{section.empty}</Text> : facts.map((fact) => {
          ...
          return <View key={fact.factId} style={[styles.fact, fact.kind === 'dealbreaker' ? styles.dealbreaker : null]}>
            ...
            <Pressable accessibilityRole="button" aria-expanded={open} accessibilityState={{ expanded: open }} onPress={() => setExpanded(open ? null : fact.factId)} testID={`sources-${fact.factId}`}><Text style={styles.sourceLink}>{open ? 'Hide submitted inputs' : `View ${fact.sourceIds.length} submitted input${fact.sourceIds.length === 1 ? '' : 's'}`}</Text></Pressable>
...
const styles = StyleSheet.create({
  ...
  back: { minHeight: 44, paddingVertical: spacing.sm, color: colors.sky, fontSize: typography.body, marginBottom: spacing.xl }, kicker: { color: colors.gold, fontSize: typography.label, fontWeight: '800', letterSpacing: 1.4 },
  title: { color: colors.ink, fontSize: typography.title, fontWeight: '700', marginTop: spacing.sm }, intro: { color: colors.textMuted, fontSize: typography.body, lineHeight: 24, marginTop: spacing.md },
  status: { borderRadius: radius.lg, borderWidth: 1, gap: spacing.sm, marginTop: spacing.xl, padding: spacing.xl }, matched: { backgroundColor: colors.surfaceTint, borderColor: colors.sky }, blocked: { backgroundColor: colors.errorSurface, borderColor: colors.danger },
  statusTitle: { color: colors.ink, fontSize: typography.heading, fontWeight: '800' }, body: { color: colors.textMuted, fontSize: typography.small, lineHeight: 20 }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm, marginTop: spacing.sm }, blockingChip: { backgroundColor: colors.errorSurface, borderRadius: radius.pill, color: colors.danger, fontSize: typography.label, fontWeight: '700', overflow: 'hidden', paddingHorizontal: spacing.md, paddingVertical: spacing.sm },
