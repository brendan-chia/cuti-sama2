...
export function DestinationsScreen({ tripId, onBack, onContinue, loadAction = loadDestinations }: Props) {
  ...
  return <Screen footer={onContinue && result.destinations.length > 0 ? <AppButton label="Continue to planning" onPress={onContinue} testID="continue-destination" /> : undefined} testID="destinations-screen">
...
const styles = StyleSheet.create({
  ...
  kicker: { color: colors.coral, fontSize: typography.label, fontWeight: '900', letterSpacing: 1.5 }, title: { color: colors.white, fontSize: typography.title, fontWeight: '900', marginTop: spacing.sm }, intro: { color: colors.textMuted, fontSize: typography.body, lineHeight: 24, marginTop: spacing.md },
  list: { gap: spacing.lg, marginTop: spacing.xxl }, noMatch: { backgroundColor: colors.midnightRaised, borderColor: colors.danger, borderRadius: radius.lg, borderWidth: 1, gap: spacing.sm, marginTop: spacing.xl, padding: spacing.xl }, noMatchTitle: { color: colors.white, fontSize: typography.heading, fontWeight: '900' }, body: { color: colors.textMuted, fontSize: typography.small, lineHeight: 20 }, categories: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm, marginTop: spacing.sm }, category: { backgroundColor: colors.coral, borderRadius: radius.pill, color: colors.midnight, fontSize: 10, fontWeight: '900', overflow: 'hidden', paddingHorizontal: spacing.md, paddingVertical: spacing.sm },
