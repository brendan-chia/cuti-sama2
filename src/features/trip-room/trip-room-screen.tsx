...
import { CardHand, ThrowingCard } from '@/features/cards/card-hand';
...
export function TripRoomScreen({ tripId, onBack, onReveal, loadAction = loadTripRoom, submitAction = submitPreferenceCard, manageAction = managePreferenceRound, subscribeAction = subscribeToTripRoom }: Props) {
  ...
  const [customOpen, setCustomOpen] = useState(false); const [dirty, setDirty] = useState(false); const [busy, setBusy] = useState(false); const [connected, setConnected] = useState(false); const [dropActive, setDropActive] = useState(false); const [error, setError] = useState<string | null>(null);
  const loadedSubmission = useRef<string | null>(null); const reducedMotion = useReducedMotion();
  const acceptRoom = useCallback((next: TripRoom) => {
    setRoom(next); const own = next.currentRound?.ownSubmission; const key = next.currentRound ? `${next.currentRound.roundId}:${own?.updatedAt ?? 'none'}` : null;
    if (!dirty || key !== loadedSubmission.current) { setSelectedId(own?.choiceId ?? null); setCustomText(own?.customText ?? ''); setDirty(false); loadedSubmission.current = key; }
  }, [dirty]);
  ...
  async function submit() {
    const round = room?.currentRound; const selected = round && selectedId ? preferenceChoiceFor(round.kind, selectedId) : null;
    if (!round || !selected) { setError('Choose one card before throwing.'); return; }
    if (selected.id === 'custom' && !customText.trim()) { setCustomOpen(true); return; }
    setBusy(true); setError(null);
    try {
      const next = await submitAction({ tripId, roundId: round.roundId, roundType: round.kind, choiceId: selected.id, customText: selected.id === 'custom' ? customText.trim() : null });
      acceptAuthoritative(next); Vibration.vibrate(12);
    } catch (cause) { setError(actionError(cause)); }
    finally { setBusy(false); }
  }
  function select(card: PreferenceChoice) { if (card.custom) { setCustomOpen(true); return; } setSelectedId(card.id); setCustomText(''); setDirty(true); setError(null); }
  ...
  const cards = round ? choicesForRound(round.kind) : []; const selected = round && selectedId ? preferenceChoiceFor(round.kind, selectedId) : null;
  return <Screen footer={<View style={styles.footer}>
    ...
    {collecting && selected ? <AppButton label={round.ownSubmission ? 'Replace my card' : 'Throw card'} loading={busy} onPress={() => void submit()} testID="throw-card" /> : null}
    ...
    {round ? <>
      ...
      {collecting ? <><Text style={styles.roundTitle}>{roundCopy[round.kind].title}</Text><Text style={styles.roundInstruction}>{roundCopy[round.kind].instruction}</Text><SharedTable active={dropActive} ownSubmitted={Boolean(round.ownSubmission)} participantCount={round.participantCount} submittedCount={round.submittedCount} />
        {selected ? <ThrowingCard card={selected} customText={selected.id === 'custom' ? customText : null} disabled={busy} onDropActive={setDropActive} onThrow={() => void submit()} reducedMotion={reducedMotion} /> : <Text style={styles.pickPrompt}>Choose a card from your hand to play.</Text>}
        {selected?.id === 'custom' && customText ? <Pressable accessibilityRole="button" onPress={() => setCustomOpen(true)} style={styles.edit}><Text style={styles.editText}>Edit custom card</Text></Pressable> : null}
        {cards.length ? <CardHand cards={cards} customText={customText} disabled={busy} onSelect={select} selectedId={selectedId} /> : <Text accessibilityRole="alert" style={styles.error}>Card choices could not be loaded.</Text>}
