...
export function ItineraryScreen({ tripId, onBack, onReview, onHome, onGroup, onMore, loadAction = loadItineraryState, generateAction = generateItinerary, slowAfterMs = 30_000, autoGenerate = false }: Props) {
  ...
  return <Screen contentStyle={styles.content} footer={footer} testID="itinerary-screen">
    ...
    {locked && !version ? <View style={styles.padded}><Text style={styles.kicker}>AI ITINERARY</Text><Text style={styles.title}>{placeLabel(locked.name)}</Text>
    ...
    {version ? <View testID="stored-itinerary">
      <View style={styles.hero}>
        <Image accessibilityLabel={`${placeLabel(version.itinerary.destination.name)} destination`} cachePolicy="memory-disk" contentFit="cover" priority="high" source={destinationPhotoUrl(version.itinerary.destination.name)} style={StyleSheet.absoluteFill} transition={240} />
        <View style={styles.heroShade} />
        <View style={styles.heroCopy}><Text style={styles.heroTitle}>{placeLabel(version.itinerary.destination.name)}</Text><View style={styles.activeChip}><Text style={styles.activeText}>Active</Text></View></View>
      </View>
      ...
      <View style={styles.days}>
        ...
        <ItineraryDays>{version.itinerary.days.map((day) => {
          ...
          return <Pressable accessibilityRole="button" key={day.dayNumber} onPress={() => onReview?.(day.dayNumber)} style={({ pressed }) => [styles.day, pressed ? styles.pressed : null]} testID={`day-${day.dayNumber}`}>
            <View style={styles.dayMeta}><View style={styles.dayHeading}><Text style={styles.dayLabel}>DAY {day.dayNumber}{day.date ? ` · ${day.date}` : ''}</Text><Text style={styles.dayTitle}>{day.title}</Text></View><View style={styles.dayPrice}><Text style={styles.dayPriceText}>{currency} {Math.round(total).toLocaleString()}</Text></View></View>
...
const styles = StyleSheet.create({
  ...
  action: { marginTop: spacing.lg, width: '100%' }, generateFooter: { gap: spacing.md }, kicker: { color: colors.coral, fontSize: typography.label, fontWeight: '700', letterSpacing: 1.5 }, title: { color: colors.ink, fontSize: typography.title, fontWeight: '700', marginTop: spacing.sm }, intro: { color: colors.textMuted, fontSize: typography.body, lineHeight: 23, marginTop: spacing.lg },
  notice: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.lg, borderWidth: 1, gap: spacing.sm, marginTop: spacing.xl, padding: spacing.xl }, sectionTitle: { color: colors.ink, fontSize: typography.heading, fontWeight: '800' }, body: { color: colors.textMuted, fontSize: 14, lineHeight: 22 }, errorBox: { backgroundColor: colors.errorSurface, borderColor: colors.danger, borderRadius: radius.lg, borderWidth: 1, marginTop: spacing.xl, padding: spacing.lg }, error: { color: colors.danger, fontSize: typography.small, lineHeight: 19, textAlign: 'center' },
  hero: { backgroundColor: colors.surfaceTint, height: 176, overflow: 'hidden', position: 'relative' }, heroShade: { backgroundColor: 'rgba(66, 46, 30, .25)', bottom: 0, left: 0, position: 'absolute', right: 0, top: 0 }, heroCopy: { bottom: spacing.lg, left: spacing.lg, position: 'absolute' }, heroTitle: { color: colors.paper, fontSize: 24, fontWeight: '700', letterSpacing: -.6, textShadowColor: 'rgba(0,0,0,.45)', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 4 }, activeChip: { alignSelf: 'flex-start', backgroundColor: colors.sky, borderRadius: radius.sm, marginTop: spacing.xs, paddingHorizontal: spacing.sm, paddingVertical: 3 }, activeText: { color: colors.paper, fontSize: 12, fontWeight: '800' },
  stats: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm, paddingHorizontal: spacing.lg, paddingVertical: spacing.md }, stat: { alignItems: 'center', backgroundColor: colors.surfaceTint, borderRadius: radius.sm, flex: 1, minWidth: 120, flexDirection: 'row', gap: 6, minHeight: 48, paddingHorizontal: spacing.sm }, statIcon: { color: colors.gold, fontSize: 15 }, statValue: { color: colors.ink, fontSize: 13, fontWeight: '700' }, statLabel: { color: colors.textMuted, fontSize: 12, marginTop: 2 },
  tabs: { flexWrap: 'wrap', borderBottomColor: colors.border, borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: 'row', gap: spacing.xl, paddingHorizontal: spacing.lg }, tab: { color: colors.textMuted, fontSize: 13, paddingBottom: spacing.md, paddingTop: spacing.sm }, activeTab: { borderBottomColor: colors.sky, borderBottomWidth: 2, color: colors.ink, fontWeight: '800' },
  days: { gap: spacing.md, paddingBottom: spacing.xl, paddingHorizontal: spacing.md, paddingTop: spacing.lg }, day: { borderWidth: 1, borderColor: colors.border, backgroundColor: colors.paper, borderRadius: radius.md, overflow: 'hidden', padding: spacing.md }, pressed: { opacity: .78, transform: [{ scale: .995 }] }, dayHeading: { flex: 1, minWidth: 140 }, dayMeta: { alignItems: 'flex-start', flexDirection: 'row', justifyContent: 'space-between', flexWrap: 'wrap', gap: spacing.sm }, dayLabel: { color: colors.sky, fontSize: 12, fontWeight: '800', letterSpacing: .7 }, dayTitle: { color: colors.ink, fontSize: typography.body, fontWeight: '800', marginTop: spacing.sm }, dayPrice: { backgroundColor: colors.background, borderColor: colors.gold, borderRadius: radius.sm, borderWidth: StyleSheet.hairlineWidth, paddingHorizontal: spacing.sm, paddingVertical: 4 }, dayPriceText: { color: colors.gold, fontSize: 12, fontWeight: '800' },
  timeline: { marginTop: spacing.md }, activity: { flexDirection: 'row', minHeight: 104 }, timelineRail: { alignItems: 'center', width: 44 }, activityIcon: { alignItems: 'center', backgroundColor: colors.leafSurface, borderRadius: radius.pill, height: 34, justifyContent: 'center', width: 34 }, activityIconWarm: { backgroundColor: colors.surfaceWarm }, activityGlyph: { color: colors.ink, fontSize: 15, fontWeight: '700' }, railLine: { backgroundColor: colors.border, flex: 1, marginVertical: 3, width: 1 }, activityCopy: { flex: 1, paddingBottom: spacing.md }, time: { color: colors.sky, fontSize: 13, lineHeight: 20, fontWeight: '600', fontVariant: ['tabular-nums'] }, activityTitle: { color: colors.ink, fontSize: 18, lineHeight: 26, fontWeight: '600', marginTop: 4 }, location: { color: colors.textMuted, fontSize: 14, lineHeight: 22, marginTop: 3 }, estimate: { alignSelf: 'flex-start', backgroundColor: colors.surfaceTint, borderRadius: 4, color: colors.gold, fontSize: 12, fontWeight: '800', marginTop: 5, overflow: 'hidden', paddingHorizontal: 5, paddingVertical: 2 }, warning: { color: colors.gold, fontSize: typography.small, lineHeight: 18 }, source: { color: colors.textMuted, fontSize: 12, lineHeight: 15, marginTop: spacing.md, textAlign: 'center' },
});
