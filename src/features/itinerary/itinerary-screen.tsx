...
export function ItineraryScreen({ tripId, onBack, onReview, loadAction = loadItineraryState, generateAction = generateItinerary, slowAfterMs = 30_000, autoGenerate = false }: Props) {
  ...
  const footer = version ? (onReview ? <AppButton label="View day details" onPress={() => onReview(1)} testID="review-itinerary" variant="secondary" /> : undefined) : locked && !error && state.currentRole === 'organizer' ? <View style={styles.generateFooter}>
  ...
  return <Screen contentStyle={styles.content} footer={footer} testID="itinerary-screen">
    ...
    {version && error ? <View style={styles.errorBox}><Text accessibilityRole="alert" style={styles.error}>{error} Your saved plan is still available.</Text><AppButton label="Retry refresh" variant="secondary" onPress={() => void refresh()} /></View> : null}
    {version ? <View testID="stored-itinerary">
      <View style={styles.hero}>
        ...
        <View style={styles.heroCopy}><Text style={styles.heroTitle}>{version.itinerary.destination.name}</Text><View style={styles.activeChip}><Text style={styles.activeText}>Draft itinerary</Text></View></View>
      </View>
      <View style={styles.stats}>{(() => { const stats = itineraryStats(version); return <>
        ...
        <View style={styles.stat}><Text style={styles.statIcon}>♧</Text><View><Text style={styles.statValue}>{stats.budget}</Text><Text style={styles.statLabel}>Activities estimate</Text></View></View>
      ...
      <View style={styles.days}>
        ...
        <Text style={styles.source}>Draft plan · Prices are estimates. Check opening hours and accessibility for each place before booking.</Text>
      </View>
    </View> : null}
  </Screen>;
}
