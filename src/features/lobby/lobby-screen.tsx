...
export function LobbyScreen({ tripId, onInvite, onAccessRevoked, onIdentityLost, onConstraints, onPreferences, onQuest, loadAction = loadLobby, readyAction = setLobbyReady, removeAction = removeLobbyMember, startAction = startTripPlanning, closeAction = closeInvitation, subscribeAction = subscribeToLobby }: Props) {
  ...
  const acceptLobby = useCallback((next: Lobby) => {
    setLobby(next);
  }, []);
  ...
  function confirmRemove(member: LobbyMember) {
    Alert.alert(`Remove ${memberLabel(member)}?`, 'They will immediately lose access and must be invited again.', [
      ...
      { text: 'Remove', style: 'destructive', onPress: () => {
        ...
        void mutate(() => removeAction(tripId, member.memberId)).finally(() => setRemovingMemberId(null));
      } },
    ]);
  }
  ...
  return <Screen footer={<View style={styles.footerActions}>
    ...
    {started && onQuest ? <AppButton label="Continue planning" onPress={onQuest} testID="open-trip-quest" /> : null}
    ...
    {started ? <View style={styles.startedBanner}><Text style={styles.startedTitle}>{onQuest ? 'Your trip quest is open' : lobby.constraintsLockedAt ? 'Constraints locked' : 'Planning has begun'}</Text><Text style={styles.body}>{onQuest ? 'Continue planning opens your shared dates, places and itinerary. Your group is set; invitations are now closed.' : lobby.constraintsLockedAt ? 'The group’s hard boundaries are ready for the next planning stage.' : `Hard constraints are next · ${constraintCount} of ${lobby.members.length} complete.`}</Text></View> : null}
...
const styles = StyleSheet.create({
  ...
  center: { alignItems: 'center', flex: 1, gap: spacing.md, justifyContent: 'center' }, headerRow: { alignItems: 'flex-start', flexDirection: 'row', justifyContent: 'space-between', flexWrap: 'wrap', gap: spacing.md }, kicker: { color: colors.sky, fontSize: typography.label, fontWeight: '800', letterSpacing: 1.7 }, tripName: { color: colors.ink, fontSize: typography.title, fontWeight: '700', marginTop: spacing.sm }, title: { color: colors.ink, fontSize: typography.heading, fontWeight: '800', textAlign: 'center' }, mode: { color: colors.textMuted, fontSize: typography.body, marginTop: spacing.sm }, connection: { alignItems: 'center', flexDirection: 'row', gap: spacing.xs }, connectionDot: { backgroundColor: colors.disabled, borderRadius: radius.pill, height: 8, width: 8 }, connectionText: { color: colors.textMuted, fontSize: typography.small }, online: { backgroundColor: colors.sky }, playerRow: { gap: spacing.lg, paddingVertical: spacing.xxl }, player: { alignItems: 'center', width: 88 }, avatar: { alignItems: 'center', backgroundColor: colors.surfaceTint, borderColor: colors.border, borderRadius: radius.pill, borderWidth: 2, height: 62, justifyContent: 'center', position: 'relative', width: 62 }, avatarReady: { borderColor: colors.sky }, avatarText: { color: colors.ink, fontSize: typography.heading, fontWeight: '700' }, presenceDot: { backgroundColor: colors.disabled, borderColor: colors.background, borderRadius: radius.pill, borderWidth: 2, bottom: 0, height: 13, position: 'absolute', right: 0, width: 13 }, playerName: { color: colors.ink, fontSize: typography.small, fontWeight: '700', marginTop: spacing.sm, maxWidth: 88 }, readyLabel: { color: colors.textMuted, fontSize: typography.label, fontWeight: '800', marginTop: spacing.xs }, ready: { color: colors.sky }, table: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.lg, borderWidth: 1, gap: spacing.sm, padding: spacing.xl }, tableKicker: { color: colors.coral, fontSize: typography.label, fontWeight: '800', letterSpacing: 1.4 }, tableTitle: { color: colors.ink, fontSize: typography.heading, fontWeight: '800' }, body: { color: colors.textMuted, fontSize: typography.body, lineHeight: 23 }, rosterHeader: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginTop: spacing.xxl }, sectionTitle: { color: colors.ink, fontSize: typography.heading, fontWeight: '800' }, count: { color: colors.textMuted, fontSize: typography.small }, roster: { borderTopColor: colors.border, borderTopWidth: StyleSheet.hairlineWidth, marginTop: spacing.md }, memberRow: { alignItems: 'center', borderBottomColor: colors.border, borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: 'row', gap: spacing.md, minHeight: 68, paddingVertical: spacing.md }, memberInfo: { flex: 1 }, memberName: { color: colors.ink, fontSize: typography.body, fontWeight: '700' }, memberMeta: { color: colors.textMuted, fontSize: typography.small, marginTop: spacing.xs }, remove: { color: colors.danger, fontSize: typography.small, fontWeight: '800', padding: spacing.md }, removeDisabled: { opacity: 0.48 }, organizerActions: { gap: spacing.md, marginTop: spacing.xl }, closed: { color: colors.textMuted, fontSize: typography.small, textAlign: 'center' }, startReason: { color: colors.textMuted, fontSize: typography.small, lineHeight: 19, marginTop: spacing.lg, textAlign: 'center' }, error: { color: colors.danger, fontSize: typography.small, lineHeight: 19, marginTop: spacing.lg, textAlign: 'center' }, footerActions: { gap: spacing.md }, startedBanner: { backgroundColor: colors.surface, borderColor: colors.gold, borderLeftWidth: 3, borderRadius: radius.md, gap: spacing.xs, marginTop: spacing.xl, padding: spacing.lg }, startedTitle: { color: colors.gold, fontSize: typography.body, fontWeight: '800' },
});
