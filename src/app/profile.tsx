...
export default function Profile() {
  ...
  return <Screen><View style={s.stack}>
    <Text style={s.title}>Your travel passport</Text>
    <AppButton label="Saved inspiration" variant="secondary" onPress={() => router.push('/inspiration')} />
    <Text style={s.body}>Keep your favourites, memories and next adventures together.</Text>
    {message ? <Text accessibilityRole="alert" style={s.body}>{message}</Text> : null}
    {!loaded ? <AppButton label="Retry loading profile" disabled={busy} onPress={() => void run(refresh)} /> : <>
    <View style={s.panel}>
    ...
    <View style={s.panel}><Text style={s.heading}>Keep your account across devices</Text><Text style={s.body}>Link this guest profile to an email and password. Confirm the email before signing in elsewhere.</Text>
    ...
    {loaded ? <>
    <Text style={s.heading}>My trips & memories</Text>
    {!trips.length ? <Text style={s.body}>Your trips will appear here when you start or join one.</Text> : trips.map(trip => <View key={trip.id} style={s.panel}>
      <Text style={s.heading}>{trip.name}</Text><Text style={s.small}>{trip.travel_party === 'solo' ? 'Solo adventure' : 'Group trip'}{completed.has(trip.id) ? ' · Completed' : ''}</Text>
      <AppButton label="Open trip" variant="secondary" onPress={() => router.push({ pathname: '/trip/[tripId]', params: { tripId: trip.id } })} />
      {!completed.has(trip.id) ? <AppButton label="Record as completed" disabled={busy} variant="secondary" onPress={() => void run(async () => { await travellerRpc('record_completed_trip', { p_trip_id: trip.id }); await refresh(); }, 'Trip added to your memories.')} /> : null}
      {!trip.planning_started_at && trip.travel_party !== 'solo' ? <AppButton label="Manage public listing" variant="secondary" onPress={() => router.push({ pathname: '/discover', params: { tripId: trip.id } })} /> : null}
    </View>)}
    </> : null}
  </View></Screen>;
}
